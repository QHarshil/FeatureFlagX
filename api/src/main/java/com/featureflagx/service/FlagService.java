...
public class FlagService {
     ...
     * This method implements caching for high performance.
     */
    public boolean isEnabled(String key, String targetId) {
        ...
        try {
            ...
            if (cachedValue != null) {
                logger.debug("Flag evaluation cache hit: {} = {}", key, cachedValue);
                return cachedValue;
            }
            ...
            logger.debug("Flag evaluation cache miss: {} = {} (cached)", key, enabled);
            return enabled;
        } catch (Exception e) {
            logger.warn("Redis cache error for flag {}, falling back to database: {}", key, e.getMessage());
            // Fallback to database if Redis is unavailable
            Optional<Flag> flagOpt = flagRepository.findByKeyIgnoreCase(key);
            return flagOpt.map(Flag::isEnabled).orElse(false);
        }
    }
    ...
    private void clearFlagCache(String key) {
        try {
            ...
            logger.warn("Failed to clear cache for flag {}: {}", key, e.getMessage());
        }
    }
}
